...
CGameTimer::CGameTimer()
{
	if (::QueryPerformanceFrequency((LARGE_INTEGER*)&m_nPerformanceFrequency)) {
		...
		//m_nLastTime = ::timeGetTime();	// 따라하기(04) 내용, 입력 시 ::GetSystemTimeAsFileTime()로 바뀜
		m_nLastTime = ::GetTickCount64();	// ChatGPT가 알려준 함수
		m_fTimeScale = 0.001f;
	}
...
void CGameTimer::Tick(float fLockFPS)
{
	if (m_bHardwareHasPerformanceCounter) {
		...
		//m_nCurrentTime = ::timeGetTime();		// 따라하기(04) 내용, 입력 시 ::GetSystemTimeAsFileTime()로 바뀜
		m_nCurrentTime = ::GetTickCount64();	// ChatGPT가 알려준 함수
	}
