...
void CGameFramework::OnDestroy()
{
	...
	for (int i = 0; i < m_nSwapChainBuffers; i++)
		if (m_ppd3RenderTargetBuffers[i])
			m_ppd3RenderTargetBuffers[i]->Release();
...
void CGameFramework::CreateSwapChain()
{
	...
	dxgiSwapChainDesc.SampleDesc.Quality = (m_bMsaa4xEnable) ? (m_uMsaa4xQualityLevels - 1) : 0;
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	m_uMsaa4xQualityLevels = d3dMsaaQualityLevels.NumQualityLevels;
	// 디바이스가 지원하는 다중 샘플의 품질 수준을 확인하다.
	m_bMsaa4xEnable = (m_uMsaa4xQualityLevels > 1) ? true : false;
...
void CGameFramework::CreateDepthStencilView()
{
	...
	d3dResourceDesc.SampleDesc.Quality = (m_bMsaa4xEnable) ? (m_uMsaa4xQualityLevels - 1) : 0;
...
void CGameFramework::FrameAdvance()
{
	...
	d3dResourceBarrier.Transition.pResource =
		m_ppd3RenderTargetBuffers[m_nSwapChainBufferIndex];
	...
	// 원하는 겂으로 깊이-스텐실(뷰)을 지운다.

	m_pd3dCommandList->OMSetRenderTargets(1, &d3dRtvCPUDescriptorHandle, TRUE, &d3dDsvCPUDescriptorHandle);
	// 렌더 타겟 뷰(서술자)와 깊이-스텔라 뷰(서술자)를 출력-병합 단계(OM)에 연결한다.
	...
	/* 현재 렌더 타겟에 대한 렌더링이 끝나기를 기다린다. GPU가 렌더 타겟(버퍼)를 더 이상 사용하지 않으면,
	...
	/* 스왑체인을 프리젠트한다. 프리젠트를 하면, 현재 렌더 타겟(후면버퍼)의 내용이 전면 버퍼로 옮겨지고
	렌더 타겟 인덱스가 바뀔 것이다. */
