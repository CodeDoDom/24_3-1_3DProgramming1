...
void CGameFramework::DeleteMenu()
{
	if (m_pMenu)
	{
		m_pMenu->ReleaseObjects();
		delete m_pMenu;
		m_pMenu = NULL;
...
void CGameFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		if (nMessageID == WM_RBUTTONDOWN)
		{
			if (m_pMenu)
			{
				m_pLockedObject = m_pMenu->PickObjectPointedByCursor(LOWORD(lParam), HIWORD(lParam), m_pPlayer->m_pCamera);
				if (m_pLockedObject) 
					DeleteMenu();
			}
...
void CGameFramework::AnimateObjects()
{
	...
	if (m_pMenu) m_pMenu->Animate(fTimeElapsed);
}
...
{    
	...
	if (m_pMenu) m_pMenu->Render(m_hDCFrameBuffer, pCamera);
